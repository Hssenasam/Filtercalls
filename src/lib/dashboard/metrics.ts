...
export const loadDashboardMetrics = async (dbArg?: D1DatabaseLike): Promise<DashboardMetrics> => {
  ...
  const windows = await Promise.all(
    WINDOW_CONFIG.map(async (window) => {
      const row = await db
        .prepare(
          'SELECT COUNT(*) as total, AVG(risk_score) as avg_risk, SUM(CASE WHEN risk_score >= 60 THEN 1 ELSE 0 END) as high_count FROM analyses WHERE created_at >= ?'
        )
        .bind(now - window.ms)
        .first<{ total: number; avg_risk: number | null; high_count: number | null }>();
  ...
  const topCountries = (
    await db
      .prepare('SELECT COALESCE(country, "Unknown") as country, COUNT(*) as count FROM analyses GROUP BY COALESCE(country, "Unknown") ORDER BY count DESC LIMIT 5')
      .bind()
  ...
  const recentRows = (
    await db
      .prepare('SELECT id, e164, COALESCE(country, "Unknown") as country, COALESCE(risk_score, 0) as risk_score, COALESCE(risk_level, "unknown") as risk_level, created_at FROM analyses ORDER BY created_at DESC LIMIT 20')
      .bind()
      .all<{ id: string; e164: string; country: string; risk_score: number; risk_level: string; created_at: number }>()
  ).results;
