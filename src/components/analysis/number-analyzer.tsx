...
export const NumberAnalyzer = ({ compact = false, initialNumber = '', autoRun = true }: NumberAnalyzerProps) => {
  ...
  const runAnalysis = useCallback(async (overrideNumber?: string) => {
    ...
    try {
      ...
      setResult(null);
      setError('We could not process this number right now. Please try again in a few seconds.');
    } finally {
      setLoading(false);
    }
  }, [country, number]);
    ...
    <div className="space-y-6">
      <Card className={compact ? 'border border-white/10 bg-white/[0.03]' : undefined}>
        <div className="space-y-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h2 className="text-lg font-semibold text-white">
                {compact ? 'Run another analysis' : 'Analyze a phone number'}
              </h2>
            </div>
            {initialNumber ? (
              <span className="inline-flex w-fit items-center rounded-full border border-violet-500/20 bg-violet-500/10 px-3 py-1 text-xs font-medium text-violet-200">
          ...
          {initialNumber && !loading && !result && !error ? (
            <p className="text-sm text-white/45">
              Loaded <span className="font-mono text-white/70">{initialNumber}</span> from the homepage.
              {autoRun ? ' Your report starts automatically.' : ' You can analyze it without retyping.'}
            </p>
          ) : null}
        ...
        <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          ...
          {result ? (
            <p className="text-xs text-white/35">
              Latest lookup: <span className="font-mono text-white/60">{number}</span>
            </p>
          ) : null}
        </div>
          ...
          <Card className="border border-white/10 bg-white/[0.03]">
            <div className="flex flex-col items-start gap-4 text-left sm:flex-row sm:items-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05]">
                <PhoneCall className="h-5 w-5 text-white/45" />
              </div>
              <div className="space-y-1">
                <p className="text-base font-medium text-white">Enter a number above to generate a report.</p>
                <p className="text-sm text-white/45">
                  We will return trust scoring, risk signals, and recommended action in one workspace.
                </p>
              </div>
            </div>
          </Card>
